...
class ILPO():
    ...
    def create_model(self, train_inputs, train_targets,train_actions, test_inputs, test_targets, test_actions):
        """ Initializes ILPO model and losses."""
        global_step = tf.contrib.framework.get_or_create_global_step()
        incr_global_step = tf.assign(global_step, global_step + 1)
        inputs = tf.concat([train_inputs, test_inputs], 0)
        targets = tf.concat([train_targets, test_targets], 0)
        with tf.variable_scope("ilpo_loss"):
            out_channels = int(targets.get_shape()[-1])
            expected_outputs, outputs, actions = self.create_ilpo(inputs, out_channels)
            out_temp=[]
            for out in outputs:
                out_temp.append(tf.split(out,num_or_size_splits=[tf.shape(train_inputs)[0],tf.shape(test_inputs)[0]],axis=0)[0])
            outputs=out_temp
            expected_outputs,test_outputs=tf.split(expected_outputs,num_or_size_splits=[tf.shape(train_inputs)[0],tf.shape(test_inputs)[0]],axis=0)
            actions=tf.split(actions,num_or_size_splits=[tf.shape(train_inputs)[0],tf.shape(test_inputs)[0]],axis=0)[0]
            inputs=train_inputs
            targets=train_targets
            # compute loss on expected next state.
            delta = slim.flatten(targets - train_actions)
        ...
        with tf.name_scope("ilpo_acc"):
            correct_prediction1 = tf.equal(tf.argmax(train_actions + expected_outputs, 1), tf.argmax(train_targets, 1))
            train_acc = tf.reduce_mean(tf.cast(correct_prediction1, tf.float32))
            correct_prediction2 = tf.equal(tf.argmax( test_actions+ test_outputs, 1), tf.argmax(test_targets, 1))
            test_acc = tf.reduce_mean(tf.cast(correct_prediction2, tf.float32))
        ...
        return Model(
            ...
            outputs=[train_actions + out for out in outputs],
            expectation=train_actions + expected_outputs,
            min_output=train_actions + min_img,
            actions=actions,
    ...
    def run(self):
        ...
        test= self.load_test()
        self.train_examples(examples, test)
